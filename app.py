...
def login():
    if request.method == "POST":
        ...
        if ok:
            session.clear()
            session.update(username=u, role=role, email=email)
            flash(msg, "success")
            return redirect(url_for("dashboard"))
        flash(msg, "danger")
        return render_template("login.html", locked=locked, locked_username=u)
    return render_template("login.html")
...
def dashboard():
    ...
    if session["role"] == "USER":
        ...
        puser = InventoryController.get_user_pending_borrows(session["username"])
        history = InventoryController.get_user_loan_history(session["username"])
    else:
        preturn = InventoryController.get_pending_returns()
        pborrow = InventoryController.get_pending_borrows()
    ...
    return render_template(
        ...
        active_loans=active,
        history=history,
        pending_returns=preturn,
        pending_borrows=pborrow,
        pending_borrow_requests=puser,
        all_loans=all_loans,
...
def borrow():
    try:
        i = int(request.form["item_id"])
        q = int(request.form.get("quantity", "1"))
    except (KeyError, ValueError):
        flash("Invalid item or borrow quantity.", "danger")
        return redirect(url_for("dashboard"))
    ok, msg = InventoryController.borrow_item(session["username"], i, q)
    flash(msg, "success" if ok else "danger")
    return redirect(url_for("dashboard"))
...
def admin_delete():
    ...
    ok, msg = InventoryController.delete_bulk_items(ids)
    flash(msg, "success" if ok else "warning")
    return redirect(url_for("dashboard"))
...
def admin_reset_action():
    ids = [int(x) for x in request.form.getlist("request_ids") if x.isdigit()]
    ...
    ok, msg = AuthController.process_bulk_resets(ids, approve, pw, admin_id)
    flash(msg, "success" if ok else "danger")
    return redirect(url_for("dashboard"))
